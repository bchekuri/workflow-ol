...
@routes.route('/api/v1/job/<int:jobId>/deny', methods=['GET'])
def deny(jobId):
    token = request.args.get('_token')
    emailId = request.args.get('_emailId')
    LOGGER.debug("Deny Request started with token: %s, jobId: %s, submitter: %s", token, jobId, emailId)
    ...
    return render_template('deny_view.html')